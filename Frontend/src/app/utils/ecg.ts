...
export class ECG {
  ...
  private line: d3.Line<ECGDataPoint> | null = null;
  ...
  private displayedData: ECGDataPoint[] = [];
  private currentTime: number = 0;
  private lastUpdateTime: number = Date.now();
  private readonly timeWindowMs: number = 10000; // 5 seconds visible window
  private readonly maxDataPoints: number = 1000;

  // Control properties
  public isPlaying = signal<boolean>(true);

  // Marker management - fully encapsulated
  ...
  public addDataPoint(dataPoint: ECGDataPoint): void {
  ...
  private updateChart(): void {
    if (!this.xScale || !this.yScale || !this.path || !this.line) return;
    ...
    this.path.datum(this.displayedData).attr('d', this.line);
  ...
  private createAxes(): void {
    ...
    this.svg
      ...
      .call(d3.axisLeft(this.yScale).tickFormat((d) => `${d}mV`))
      .style('color', 'white');
  }
  ...
  // Marker management methods - fully encapsulated
  ...
  private openMarkerDialog(dataPoint: ECGDataPoint): void {
    this.selectedMarker = {
  ...
  private updateMarkerPositions(): void {
    ...
    const selection = this.svg
      .selectAll<SVGCircleElement, Marker>('circle.marker-point')
    ...
    selection
      ...
      .on('click', (event: MouseEvent, d: Marker) => {
